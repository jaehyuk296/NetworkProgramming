...
public class DavinciGameLogic {
    ...
    public void handleDraw(ClientHandler player, String data) {
        ...
        if (drawn.isJoker()) {
            ...
            player.sendMessage(new Message(Message.RES_DRAW, drawn, -1)); // 인덱스는 -1로 전송
    ...
    // 조커 위치 설정 로직 (조커를 뽑은 후 클라이언트에서 위치를 지정했을 때)
    ...
    public void handleGuess(ClientHandler guesser, String data) {
        ...
        if (isCorrect) {
            ...
            if (isPlayerOut(targetID)) {
                broadcaster.broadcastChat("[System] " + targetID + "님이 모든 타일이 공개되어 탈락하셨습니다.");
                broadcaster.roomLog(targetID + " 탈락 (패 전체 공개됨)");
            }
            ...
            // 맞췄을 때는 nextTurn()을 호출하지 않음 (유저에게 선택권 부여)
            ...
            if (currentTurnDrawnTile != null && !currentTurnDrawnTile.isRevealed()) {
                ...
                if (isPlayerOut(guesser.getNickname())) {
                    broadcaster.broadcastChat("[System] " + guesser.getNickname() + "님이 모든 타일이 공개되어 탈락하셨습니다.");
                    broadcaster.roomLog(guesser.getNickname() + " 탈락 (패 전체 공개됨)");
                }
            }
